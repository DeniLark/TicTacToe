...
import TicTacToe.Game (GameBoard, Player)

htmlTemplate :: Html -> Html
htmlTemplate html = H.html $ do
  H.head $ do
    H.title "Крестики, нолики"
    H.style styles
  H.body $ do
    H.div H.! A.class_ "container" $ html

htmlMainPage :: Player -> GameBoard -> Html
htmlMainPage player gameBoard = htmlTemplate $ do
...
htmlEndPage player gameBoard = htmlTemplate $ do
  case player of
    Nothing -> H.p "Ничья"
